...
func Run(analyzers []*analysis.Analyzer, packages ...string) ([]util.Finding, bool, error) {
	...
	resultMaps := []map[*analysis.Analyzer]interface{}{}
	...
	for _, pkg := range pkgs {
		...
		util.InitCallgraph(ssaResult.(*buildssa.SSA).SrcFuncs)
	...
	//无需在意，需要pkg作为字段，但实际上用哪个一点都不影响（因为已丢进callgraph）。随便给一个即可。
	pkg := pkgs[0]
...
func LoadPackages(packagesList ...string) ([]*packages.Package, bool, error) {
	...
	util.Prog, _ = ssautil.AllPackages(pkgs, 0)
	util.Prog.Build()
