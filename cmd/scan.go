...
func init() {
	...
	scanCmd.Flags().StringVarP(&sourceYml, "source", "i", "", "custom source yml config file path")
	scanCmd.Flags().StringVarP(&sinkYml, "sink", "n", "", "custom sink yml config file path")
	scanCmd.Flags().StringVarP(&outputPath, "output", "o", "", "file path to write findings output instead of stdout")
	scanCmd.Flags().BoolP("webMode", "w", false, "web mode, scan web project")
...
Scans a Go module directory. To scan the current directory recursively, use gokart scan. To scan a specific directory, use gokart scan <directory>.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		webMode, _ := cmd.Flags().GetBool("webMode")

		util.InitConfig(globals, sarif, json, verbose, debug, outputPath, sourceYml, sinkYml, exitCode, webMode)
