...
func getVulnSqlFuncs() map[string][]string {
	//这部分没完全测试，可能有问题。
	return util.Sinks.SqlInjectionFuncs
}
...
func sqlRun(pass *analysis.Pass) (interface{}, error) {
	...
	for pkg, funcs := range vuln_db_funcs {
		...
		for _, fn := range funcs {
			...
			for _, vulnFunc := range util.Cg[current_function] {
				...
				if taint_analyzer.ContainsTaint(&vulnFunc.Instr.Call, &vulnFunc.Instr.Call.Args[argIndex], util.Cg) {
					message := "Danger: possible SQL injection detected"
					targetFunc := util.GenerateTaintedCode(pass, vulnFunc.Fn, vulnFunc.Instr.Pos(), []int{}, false)
