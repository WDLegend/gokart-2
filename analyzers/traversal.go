...
func getVulnInjectionFunctions() map[string][]string {
	return util.Sinks.TraversalFuncs
}
...
func traversalRun(pass *analysis.Pass) (interface{}, error) {
	...
	for pkg, funcs := range pathFuncs {
		...
		for _, fn := range funcs {
			...
			for _, vulnFunc := range util.Cg[curFunc] {
				...
				if taintAnalyzer.ContainsTaint(&vulnFunc.Instr.Call, &vulnFunc.Instr.Call.Args[0], util.Cg) {
